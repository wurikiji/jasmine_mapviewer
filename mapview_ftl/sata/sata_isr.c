...
{
	UINT32 lba, sector_count, cmd_code, cmd_type;
	static volatile UINT32 fis_d0, fis_d1, fis_d2, fis_d3;
	is_share = 0;
	...
	if ((cmd_type & ATR_LBA_NOR))
	{
		...
		if (cmd_code == 0x25 && 0 != (fis_d1 & 0x0F000000))
		{
			is_share = (fis_d1 >> 24);
		}
	}
	...
	if (is_share)
	{
		SETREG(SATA_FIS_H2D_1, (fis_d1 & 0xF0FFFFFF));
	}
