...
extern unsigned int share_count, misc_share_count, gc_count, write_count, copy_back_count;
void ata_identify_device(UINT32 lba, UINT32 sector_count)
{
	UINT16* const addr = ht_identify_data;
	UINT32 capacity;

#if defined(OPTION_UART_DEBUG) && 0
	char buff[1024];
	sprintf(buff,"SHARE call %d, Misc %d, GC %d, Write %d, Copy back %d\n", 
			share_count, misc_share_count, gc_count, write_count, copy_back_count);

    uart_print(buff);

    sprintf(buff, "%d, BM_WRITE_LIMIT: %d, SATA_WBUF_PTR: %d, SATA_WBUF_FREE: %d\n",
            GETREG(SATA_BUF_PAGE_SIZE), GETREG(BM_WRITE_LIMIT), GETREG(SATA_WBUF_PTR),
            GETREG(SATA_WBUF_FREE));
    uart_print(buff);
